...
- `POST /v1/trust-roots` registers an X.509 trust root for a tenant and workload. The JSON body must include non-empty `tenant_id`, `workload_id` and `root_pem`, plus an optional non-empty `name`. `root_pem` must be a PEM-encoded X.509 CA certificate (basic constraints `CA:TRUE`); only the public certificate is persisted — private key material is never accepted, stored, or returned. On success it returns `201` with `root_id`, `tenant_id`, `workload_id`, `name` and a UTC RFC3339 `created_at`. Registering the same certificate again for the same tenant and workload returns `409`; invalid fields, unparseable PEM, or a non-CA certificate return `422`. Trust roots persist across restarts and are scoped strictly to their owning tenant and workload.
...
The built-in format `x509-attested-nonce-json` accepts the JSON object `{"nonce": "<unpadded base64url challenge nonce>", "claims": {...}, "certificate_chain": ["<leaf PEM>", ..., "<root PEM>"], "signature": "<base64>"}`. `certificate_chain` is a non-empty list of PEM certificates ordered leaf-to-root, and `signature` is the base64-encoded signature made by the leaf private key over the canonical JSON of `{"claims": ..., "nonce": ...}` (sorted keys, compact separators). After the usual identity, nonce and digest checks, the verifier checks that every certificate is within its validity period, that every certificate above the leaf carries a CA basic-constraints usage, that each certificate is issued and signed by the next, that the chain root is byte-identical to a trust root registered via `POST /v1/trust-roots` for the same tenant and workload, and that the payload signature verifies against the leaf public key. Any format, chain, trust, validity, or signature failure settles atomically as `200` with status `rejected`; certificates, evidence, and error details are never persisted, logged, or returned.
