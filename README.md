...
#### Plugin security limits

The verifier boundary is treated as untrusted output. The contract is intentionally minimal:

- `verify()` must return a `VerificationResult`, whose **only** field is the boolean `accepted`. That single bit decides whether the record settles as `verified` or `rejected`. There is no message, reason or detail field.
- The service never accepts, truncates, transforms, logs, persists or returns any plugin-supplied text. Passing an unexpected keyword (e.g. `VerificationResult(accepted=True, detail=...)`), attaching extra attributes, or returning a non-`VerificationResult` object / non-boolean verdict is treated as a plugin fault (`500`), with the record left `received` and retryable after the plugin is replaced.
- Persisted outcomes use only the service's own finite, non-sensitive code set (`accepted`/`rejected`, see `VerificationResultCode`), mapped solely from `accepted` — never from anything the plugin provides.
- Raw evidence is handed to the plugin for the duration of that one call only. A plugin exception (including a message that embeds evidence or secrets) is logged by exception **type** and non-sensitive identifiers only; the exception text and traceback are never logged or returned, the transaction is rolled back, and the record stays `received`.
- Older SQLite databases that stored a free-text verifier note migrate forward automatically: the new result-code column is added and backfilled from the settled status, and the legacy note is scrubbed (and physically purged via `VACUUM`). Starting from such a database verifies normally and never fails on, or leaks, the old note.
